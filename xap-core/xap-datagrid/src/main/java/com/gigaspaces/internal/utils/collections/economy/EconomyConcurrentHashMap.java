 ...
 *            option of unstableKey which enforces creation of pinned HashEntry object
 * @author Doug Lea
 * @since 1.5
 */
@com.gigaspaces.api.InternalApi
public class EconomyConcurrentHashMap<K, V> extends AbstractMap<K, V>
        implements IEconomyConcurrentMap<K, V>, Serializable {
    ...
    //a hashentry that is piined- used when key is unstable- the key in the
    // value object may be changed, we must use a hashentry and pin it
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
         ...
         */
        transient volatile IHashEntry<K, V>[] table;
        ...
        /**
         * Sets table to new IHashEntry array. Call only while holding lock or in constructor.
         */
        ...
        IHashEntry<K, V> getFirst(int hash) {
            IHashEntry<K, V>[] tab = table;
            return tab[hash & (tab.length - 1)];
        }

        /**
         * Reads value field of an entry under lock. Called if value field ever appears to be null.
         * This is possible only if a compiler happens to reorder a IHashEntry initialization with
        ...
        V get(Object key, int hash) {
            if (count != 0) { // read-volatile
                IHashEntry<K, V> e = getFirst(hash);
                while (e != null) {
                    if (_h.hash(e) == hash && key.equals(_h.key(e))) {
                        V v = _h.value(e);
                        if (v != null)
                            return v;
        ...
        boolean containsValue(Object value) {
            if (count != 0) { // read-volatile
                ...
                int len = tab.length;
                for (int i = 0; i < len; i++) {
                    for (IHashEntry<K, V> e = tab[i]; e != null; e = _h.next(e)) {
                        V v = _h.value(e);
                        if (v == null) // recheck
        ...
        boolean replace(K key, int hash, V oldValue, V newValue) {
            ...
            try {
                ...
                IHashEntry<K, V> first = tab[index];
                IHashEntry<K, V> e = first;
                while (e != null && (_h.hash(e) != hash || !key.equals(_h.key(e))))
                    e = _h.next(e);

                boolean replaced = false;
                if (e != null && oldValue.equals(_h.value(e))) {
                    replaced = true;
                    if (e.isNativeHashEntry()) {
                        ((HashEntry<K, V>) e).value = newValue;
                    } else {
                        //Clone the list up to and including e
                        IHashEntry<K, V> newNode = _h.createEntry(key, newValue, _h.next(e), hash);
                        IHashEntry<K, V> a = first;
                        while (a != null && a != e) {
                            newNode = _h.cloneEntry(a, newNode);
                            a = _h.next(a);
                        }
                        tab[index] = newNode;
                    }
        ...
        void setKeyUnstable(K key, int hash) {
            ...
            try {
                ...
                boolean found = false;
                if (e != null) {
                    if (e.isNativeHashEntry() && ((HashEntry<K, V>) e).isPinnedHashEntry())
                        return;
                    found = true;
                    //Clone the list up to and including e
                    IHashEntry<K, V> newNode = _h.cloneEntry(e, _h.next(e), true /*unstableKey*/);
                    IHashEntry<K, V> a = first;
                    while (a != null && a != e) {
                        newNode = _h.cloneEntry(a, newNode);
                        a = _h.next(a);
        ...
        V put(K key, int hash, V value, boolean onlyIfAbsent, boolean unstableKey) {
            ...
            try {
                ...
                if (e != null) {
                    ...
                    oldValue = null;
                    ++modCount;
                    tab[index] = _h.createEntry(key, value, first, hash, unstableKey);
        ...
        void rehash() {
            ...
            for (int i = 0; i < oldCapacity; i++) {
                ...
                if (e != null) {
                    IHashEntry<K, V> next = _h.next(e);
                    int idx = _h.hash(e) & sizeMask;
                    ...
                    if (next == null)
                        ...
                        int lastIdx = idx;
                        for (IHashEntry<K, V> last = next;
                             last != null;
                             last = _h.next(last)) {
                            int k = _h.hash(last) & sizeMask;
                            if (k != lastIdx) {
                                lastIdx = k;
                                lastRun = last;
                            }
                        ...
                        for (IHashEntry<K, V> p = e; p != lastRun; p = _h.next(p)) {
                            int k = _h.hash(p) & sizeMask;
                            IHashEntry<K, V> n = newTable[k];
                            newTable[k] = _h.cloneEntry(p, n);
            ...
            try {
                ...
                if (e != null) {
                    ...
                    if (value == null || value.equals(v)) {
                        ...
                        // cloned.
                        ++modCount;
                        IHashEntry<K, V> newFirst = _h.next(e);
                        for (IHashEntry<K, V> p = first; p != e; p = _h.next(p))
                            newFirst = _h.cloneEntry(p, newFirst);
                        tab[index] = newFirst;
                        count = c; // write-volatile
                    }
                }
     ...
     */
    public EconomyConcurrentHashMap(int initialCapacity,
                                    float loadFactor, int concurrencyLevel, IHashEntryHandler<K, V> handler) {
    ...
    public EconomyConcurrentHashMap(int initialCapacity, float loadFactor, IHashEntryHandler<K, V> handler) {
        this(initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL, handler);
    }
    ...
    public EconomyConcurrentHashMap(int initialCapacity, IHashEntryHandler<K, V> handler) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, handler);
    }
    ...
    public EconomyConcurrentHashMap(IHashEntryHandler<K, V> handler) {
        this(DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, handler);
    }
    ...
    public EconomyConcurrentHashMap(Map<? extends K, ? extends V> m, IHashEntryHandler<K, V> handler) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1,
                ...
                DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, handler);
        putAll(m);
    }
    ...
    /**
     * Legacy method testing if some key maps into the specified value in this table.  This method
     * is identical in functionality to {@link #containsValue}, and exists solely to ensure full
    ...
    public V put(K key, V value) {
        if (value == null)
            throw new NullPointerException();
        int hash = hash(key.hashCode());
        return segmentFor(hash).put(key, hash, value, false, false /*unstablekey*/);
    }
    ...
    public V putIfAbsent(K key, V value) {
        if (value == null)
            throw new NullPointerException();
        int hash = hash(key.hashCode());
        return segmentFor(hash).put(key, hash, value, true, false /*unstablekey*/);
    }
    ...
    public void setKeyUnstable(K key) {
        int hash = hash(key.hashCode());
        segmentFor(hash).setKeyUnstable(key, hash);
    ...
    abstract class HashIterator {
        ...
        final void advance() {
            if (nextEntry != null && (nextEntry = _handler.next(nextEntry)) != null)
                return;
        ...
        public void remove() {
            ...
            EconomyConcurrentHashMap.this.remove(_handler.key(lastReturned));
            lastReturned = null;
        }
    }
    ...
    final class KeyIterator
        ...
        public K next() {
            return _handler.key(super.nextEntry());
        }
    ...
    final class ValueIterator
        ...
        public V next() {
            return _handler.value(super.nextEntry());
        }
    ...
    final class WriteThroughEntry
        ...
        public V setValue(V value) {
            ...
            EconomyConcurrentHashMap.this.put(getKey(), value);
            return v;
        }
    }
    ...
    final class EntryIterator
        ...
        public Map.Entry<K, V> next() {
            IHashEntry<K, V> e = super.nextEntry();
            return new WriteThroughEntry(_handler.key(e), _handler.value(e));
        }
    }
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            ...
            V v = EconomyConcurrentHashMap.this.get(e.getKey());
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        for (int k = 0; k < segments.length; ++k) {
            ...
            try {
                ...
                for (int i = 0; i < tab.length; ++i) {
                    for (IHashEntry<K, V> e = tab[i]; e != null; e = _handler.next(e)) {
                        s.writeObject(_handler.key(e));
                        s.writeObject(_handler.value(e));
                    }
                }
            } finally {
                seg.unlock();
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        for (int i = 0; i < segments.length; ++i) {
            segments[i].setTable(new IHashEntry[1]);
        }
