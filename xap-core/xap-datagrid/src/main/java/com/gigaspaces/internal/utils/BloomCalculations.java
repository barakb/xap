...
/**
 * The following calculations are taken from: http://www.cs.wisc.edu/~cao/papers/summary-cache/node8.html
 * "Bloom Filters - the math"
 *
 * This class's static methods are meant to facilitate the use of the Bloom Filter class by helping
 * to choose correct values of 'bits per element' and 'number of hash functions, k'.
 */
class BloomCalculations {
    ...
    /**
     * In the following table, the row 'i' shows false positive rates if i buckets per element are
     * used.  Column 'j' shows false positive rates if j hash functions are used.  The first row is
     * 'i=0', the first column is 'j=0'. Each cell (i,j) the false positive rate determined by using
     * i buckets per element and j hash functions.
     */
    ...
    /**
     * Given the number of buckets that can be used per element, return a specification that
    ...
    /**
     * A wrapper class that holds two key parameters for a Bloom Filter: the number of hash
     * functions used, and the number of buckets per element used.
     */
    ...
    /**
     * Given a maximum tolerable false positive probability, compute a Bloom specification which
     * will give less than the specified false positive rate, but minimize the number of buckets per
     * element and the number of hash functions used.  Because bandwidth (and therefore total
     * bitvector size) is considered more expensive than computing power, preference is given to
     * minimizing buckets per element rather than number of hash functions.
     *
     ...
     * @return A Bloom Specification which would result in a false positive rate less than specified
