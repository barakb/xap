 ...
 * @since 9.1
 */
public final class RemoveFromCollectionSpaceEntryMutator
        extends SpaceEntryPathMutator {
    private static final long serialVersionUID = 1L;

    private Serializable _item;

    public RemoveFromCollectionSpaceEntryMutator() {
    }
    ...
    public CollectionChangeSpaceEntryMutatorResult change(MutableServerEntry entry) {
        ...
        if (oldValue == null)
            throw new IllegalStateException("No collection instance exists under the given path '" + getPath() + "', in order to remove a value a collection instance must exists");
        Collection newValue = CollectionUtils.cloneCollection(oldValue);
        boolean changed = newValue.remove(_item);
    ...
    public String getName() {
        return RemoveFromCollectionOperation.NAME;
    }
