 ...
 * support the <tt>remove</tt> method.<p>
 *
 ...
 * recommendation in the <tt>Map</tt> interface specification.<p>
 *
 * The documentation for each non-abstract methods in this class describes its implementation in
...
public abstract class AbstractMap<K, V> implements Map<K, V> {
     ...
     * <tt>Integer.MAX_VALUE</tt> elements, returns <tt>Integer.MAX_VALUE</tt>.<p>
     *
     ...
     * <tt>true</tt> if and only if this map contains at least one mapping to a value <tt>v</tt>
     * such that <tt>(value==null ? v==null : value.equals(v))</tt>.  This operation will probably
     * require time linear in the map size for most implementations of map.<p>
     *
     * This implementation iterates over entrySet() searching for an entry with the specified value.
     * If such an entry is found, <tt>true</tt> is returned.  If the iteration terminates without
     * finding such an entry, <tt>false</tt> is returned.  Note that this implementation requires
     ...
     *
     * This implementation iterates over <tt>entrySet()</tt> searching for an entry with the
     ...
     * implementation requires linear time in the size of the map; many implementations will
     * override this method.
     *
     * @param key key whose presence in this map is to be tested.
     * @return <tt>true</tt> if this map contains a mapping for the specified key.
     * @throws NullPointerException if the key is <tt>null</tt> and this map does not permit
     *                              <tt>null</tt> keys.
     */
     ...
     * contains no mapping for this key.  A return value of <tt>null</tt> does not
     * <i>necessarily</i> indicate that the map contains no mapping for the key; it's also possible
     * that the map explicitly maps the key to <tt>null</tt>.  The containsKey operation may be used
     * to distinguish these two cases. <p>
     *
     * This implementation iterates over <tt>entrySet()</tt> searching for an entry with the
     ...
     * the map previously contained a mapping for this key, the old value is replaced.<p>
     *
     ...
     * associated <tt>null</tt> with the specified key, if the implementation supports <tt>null</tt>
     * values.)
     ...
     * @throws IllegalArgumentException      if some aspect of this key or value * prevents it from
     *                                       being stored in this map.
     * @throws NullPointerException          if this map does not permit <tt>null</tt> keys or
     *                                       values, and the specified key or value is
     *                                       <tt>null</tt>.
     */
     ...
     * requires linear time in the size of the map; many implementations will override this
     * method.<p>
     *
     * Note that this implementation throws an <tt>UnsupportedOperationException</tt> if the
     * <tt>entrySet</tt> iterator does not support the <tt>remove</tt> method and this map contains
     ...
     * calls this map's <tt>put</tt> operation once for each entry returned by the iteration.<p>
     *
     ...
     * @throws IllegalArgumentException      if some aspect of a key or value in the specified map
     *                                       prevents it from being stored in this map.
     * @throws NullPointerException          if the specified map is <tt>null</tt>, or if this map
     *                                       does not permit <tt>null</tt> keys or values, and the
     ...
     * changes to the map are reflected in the Set, and vice-versa.  (If the map is modified while
     * an iteration over the Set is in progress, the results of the iteration are undefined.) The
     * Set supports element removal, which removes the corresponding entry from the map, via the
     * Iterator.remove, Set.remove,  removeAll retainAll, and clear operations.  It does not support
     * the add or addAll operations.<p>
     *
     * This implementation returns a Set that subclasses AbstractSet.  The subclass's iterator
     * method returns a "wrapper object" over this map's entrySet() iterator.  The size method
     * delegates to this map's size method and the contains method delegates to this map's
     ...
     * is modified while an iteration over the collection is in progress, the results of the
     * iteration are undefined.)  The collection supports element removal, which removes the
     * corresponding entry from the map, via the <tt>Iterator.remove</tt>,
     * <tt>Collection.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt> and <tt>clear</tt>
     * operations. It does not support the <tt>add</tt> or <tt>addAll</tt> operations.<p>
     *
     * This implementation returns a collection that subclasses abstract collection.  The subclass's
     * iterator method returns a "wrapper object" over this map's <tt>entrySet()</tt> iterator.  The
     * size method delegates to this map's size method and the contains method delegates to this
     * map's containsValue method.<p>
     *
     ...
     * vice-versa.  (If the map is modified while an iteration over the set is in progress, the
     * results of the iteration are undefined.)  The set supports element removal, which removes the
     * corresponding entry from the map, via the <tt>Iterator.remove</tt>, <tt>Set.remove</tt>,
     ...
     * <tt>t1</tt> and <tt>t2</tt> represent the same mappings if <tt>t1.keySet().equals(t2.keySet())</tt>
     * and for every key <tt>k</tt> in <tt>t1.keySet()</tt>, <tt> (t1.get(k)==null ? t2.get(k)==null
     * : t1.get(k).equals(t2.get(k))) </tt>.  This ensures that the <tt>equals</tt> method works
     * properly across different implementations of the map interface.<p>
     *
     ...
     * the size of this set; if not, it returns <tt>false</tt>.  If so, it iterates over this map's
     ...
     * <tt>t1</tt> and <tt>t2</tt>, as required by the general contract of Object.hashCode.<p>
     *
     * This implementation iterates over <tt>entrySet()</tt>, calling <tt>hashCode</tt> on each
     ...
     * "</tt> (comma and space).  Each key-value mapping is rendered as the key followed by an
     * equals sign (<tt>"="</tt>) followed by the associated value.  Keys and values are converted
     * to strings as by <tt>String.valueOf(Object)</tt>.<p>
     *
     * This implementation creates an empty string buffer, appends a left brace, and iterates over
