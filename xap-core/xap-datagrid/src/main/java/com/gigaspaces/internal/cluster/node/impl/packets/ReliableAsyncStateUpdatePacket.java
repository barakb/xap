...
public class ReliableAsyncStateUpdatePacket
    ...
    public Object accept(IIncomingReplicationFacade incomingReplicationFacade) {
        IReplicationTargetGroup replicationTargetGroup = incomingReplicationFacade.getReplicationTargetGroup(getGroupName());
        IReplicationTargetChannel targetChannel = replicationTargetGroup.getChannel(getSourceLookupName());
        IReplicationReliableAsyncKeeperTargetProcessLog processLog = (IReplicationReliableAsyncKeeperTargetProcessLog) targetChannel.getProcessLog();
        IReplicationReliableAsyncMediator mediator = processLog.getMediator();
        mediator.updateReliableAsyncState(_reliableAsyncState, getSourceLookupName());
        return null;
    }

    public void readExternalImpl(ObjectInput in, PlatformLogicalVersion endpointLogicalVersion) throws IOException,
            ClassNotFoundException {
        _reliableAsyncState = IOUtils.readObject(in);
    }

    public void writeExternalImpl(ObjectOutput out, PlatformLogicalVersion endpointLogicalVersion) throws IOException {
        IOUtils.writeObject(out, _reliableAsyncState);
    }
