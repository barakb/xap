...
public class PrimarySpaceRecovery
     ...
     * @param recoveryGroup
     */
    public PrimarySpaceRecovery(SpaceImpl space) {
        _space = space;
        _recoveryGroup = new CompositeRecoveryGroup(space);

        ClusterPolicy clusterPolicy = space.getClusterPolicy();
        if (clusterPolicy == null)
            return;

        // Build internal components        
        if (clusterPolicy.m_ReplicationPolicy != null)
            _recoveryGroup.add(new ReplicationGroupRecovery(space));

        if (clusterPolicy.m_LoadBalancingPolicy != null)
            _recoveryGroup.add(new LoadBalancingGroupRecovery(space));
