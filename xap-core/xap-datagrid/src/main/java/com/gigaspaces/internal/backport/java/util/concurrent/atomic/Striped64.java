...
package com.gigaspaces.internal.backport.java.util.concurrent.atomic;
...
abstract class Striped64 extends Number {
    private static final long serialVersionUID = -4854391916500231993L;
     ...
     * between pads, hoping that the JVM doesn't reorder them. <p/> JVM intrinsics note: It would be
     * possible to use a release-only form of CAS here, if it were provided.
     */
    static final class Cell {
        ...
        final boolean cas(long cmp, long val) {
            return UnsafeHolder.compareAndSwapLong(this, valueOffset, cmp, val);
        }
        ...
        static {
            try {
                Class<?> ak = Cell.class;
                valueOffset = UnsafeHolder.objectFieldOffset(ak.getDeclaredField("value"));
            } catch (Exception e) {
                throw new Error(e);
            }
        }
    ...
    final boolean casBase(long cmp, long val) {
        return UnsafeHolder.compareAndSwapLong(this, baseOffset, cmp, val);
    }
    ...
    final boolean casBusy() {
        return UnsafeHolder.compareAndSwapInt(this, busyOffset, 0, 1);
    }
    ...
    static {
        try {
            ...
            baseOffset = UnsafeHolder.objectFieldOffset(sk.getDeclaredField("base"));
            busyOffset = UnsafeHolder.objectFieldOffset(sk.getDeclaredField("busy"));
        } catch (Exception e) {
            throw new Error(e);
        }
    }
