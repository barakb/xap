...
/**
 * Simple implementation of <code>LeasePeriodPolicy</code> that grants lease times based on a fixed
 * default and maximum lease. Will grant renewals longer than the maximum if the current lease and
 * the request are both longer than the maximum.
 *
...
public class FixedLeasePeriodPolicy implements LeasePeriodPolicy {
    ...
    /**
     * Create a new <code>FixedLeasePeriodPolicy</code> with the specified values for the maxium and
     * default lease lengths.
     *
     * @param maximum       the length in milliseconds of the longest lease this object should
     *                      normally grant
     * @param defaultLength the length in milliseconds of the default lease grants
     ...
     * The duration actually granted will be shorter if the duration extends pass the end of the
     * epoch.  Must return a positive number. <p> Note the duration returned by this method will be
     * shorter than the final duration granted if the requested duration extends past the current
     * expiration, and duration return by this method is before the current expiration.
     *
     * @param resource          the resource having a lease granted or renewed
     * @param requestedDuration the duration the client wants
     * @throws IllegalArgumentException thrown if requestedDuration is less than 0 and not equal to
     *                                  <code>Lease.ANYLENGTH</code> or <code>Lease.FOREVER</code>.
     */
     ...
     * the epoch. Default implementation calls System.currentTimeMillis()
     */
    ...
    /**
     * Calculates an expiration based on the passed time and a requested duration. Will clip the
     * expiration to the end of the epoch
     *
     * @param preferredDuration the duration the policy wants to grant
     * @param now               the current time in milliseconds since the beginning of the epoch
     * @return the new expiration time for the lease in milliseconds since the beginning of the
     * epoch
     */
