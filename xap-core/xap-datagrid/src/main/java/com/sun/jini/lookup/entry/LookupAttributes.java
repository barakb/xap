...
/**
 * Some simple utilities for manipulating lookup service attributes. These are not high-performance
 * operations; it is expected that they are called relatively infrequently.
 *
...
public class LookupAttributes {
    ...
    /**
     * Returns a new array containing the elements of the <code>addAttrSets</code> parameter (that
     * are not duplicates of any of the elements already in the <code>attrSets</code> parameter)
     * added to the elements of <code>attrSets</code>. The parameter arrays are not modified. <p>
     * Note that attribute equality is defined in terms of <code>MarshalledObject.equals</code> on
     * field values.  The parameter arrays are not modified. <p> Throws an
     * <code>IllegalArgumentException</code> if any element of <code>addAttrSets</code> is not an
     * instance of a valid <code>Entry</code> class (the class is not public, or does not have a
     ...
     * added to the elements of <code>attrSets</code>. The parameter arrays are not modified. <p>
     * Note that attribute equality is defined in terms of <code>MarshalledObject.equals</code> on
     * field values.  The parameter arrays are not modified. <p> If the <code>checkSC</code>
     * parameter is <code>true</code>, then a <code>SecurityException</code> is thrown if any
     * elements of the <code>addAttrSets</code> parameter are instanceof
     * <code>ServiceControlled</code>. <p> Throws an <code>IllegalArgumentException</code> if any
     * element of <code>addAttrSets</code> is not an instance of a valid <code>Entry</code> class
    ...
    /**
     * Returns a new array that contains copies of the attributes in the <code>attrSets</code>
     * parameter, modified according to the contents of both the <code>attrSetTmpls</code> parameter
     * and the <code>modAttrSets</code> parameter. The parameter arrays and their <code>Entry</code>
     * instances are not modified. <p> Throws an <code>IllegalArgumentException</code> if any
     * element of <code>attrSetTmpls</code> or <code>modAttrSets</code> is not an instance of a
     * valid <code>Entry</code> class (the class is not public, or does not have a no-arg
     ...
     * parameter, modified according to the contents of both the <code>attrSetTmpls</code> parameter
     * and the <code>modAttrSets</code> parameter. The parameter arrays and their <code>Entry</code>
     * instances are not modified. <p> If the <code>checkSC</code> parameter is <code>true</code>,
     * then a <code>SecurityException</code> is thrown if any elements of the <code>attrSets</code>
     * parameter that would be deleted or modified are instanceof <code>ServiceControlled</code>.
     * <p> Throws an <code>IllegalArgumentException</code> if any element of
     * <code>attrSetTmpls</code> or <code>modAttrSets</code> is not an instance of a valid
     * <code>Entry</code> class (the class is not public, or does not have a no-arg constructor, or
    ...
    /**
     * Test that two entries are the same type, with the same public fields. Attribute equality is
     * defined in terms of <code>MarshalledObject.equals</code> on field values.
     */
    public static boolean equal(Entry e1, Entry e2) {
        // Test if same:
        if (e1 == e2)
            return true;
        // Test if either is null:
        if (e1 == null || e2 == null)
            return false;
        // Test if same type:
    ...
    /**
     * Test if the parameter <code>tmpl</code> is the same class as, or a superclass of, the
     * parameter <code>e</code>, and that every non-<code>null</code> public field of
     * <code>tmpl</code> is the same as the corresponding field of <code>e</code>. Attribute
     * equality is defined in terms of <code>MarshalledObject.equals</code> on field values.
     */
    ...
    /**
     * Throws an <code>IllegalArgumentException</code> if any element of the array is not an
     * instance of a valid <code>Entry</code> class (the class is not public, or does not have a
     * no-arg constructor, or has primitive public non-static non-final fields).  If
     * <code>nullOK</code> is <code>false</code>, and any element of the array is <code>null</code>,
    ...
    public static void check(Entry[] attrs, boolean nullOK) {
        for (int i = attrs.length; --i >= 0; ) {
            ...
            if (e == null) {
                if (nullOK)
                    continue;
                throw new IllegalArgumentException("attrs[" + i + "] is null.");
            }
            Class c = e.getClass();
    ...
    /**
     * Throws a <code>SecurityException</code> if parameter <code>e</code> is instanceof
    ...
    /**
     * Test if the set at the given <code>index</code> is equal to any other set earlier in the
     * <code>Entry[]</code> array parameter.
     */
    ...
    /**
     * Return a new entry that, for each non-<code>null</code> field of the parameter
     * <code>mods</code>, has the same field value as <code>mods</code>, else the same field value
     * as the parameter <code>e</code>.
     */
    private static Entry update(Entry e, Entry mods) {
        try {
            Entry ec = e.getClass().newInstance();
    ...
    /**
     * Tests if two classes are equal, using the class equivalence semantics of the lookup service:
     * same name.
     */
    ...
    /**
     * Tests if class <code>c1</code> is equal to, or a superclass of, class <code>c2</code>, using
     * the class equivalence semantics of the lookup service: same name.
     */
    ...
    private static class FieldComparator implements Comparator {
        ...
        /**
         * Sorts superclass fields before subclass fields, and sorts fields alphabetically within a
         * given class.
         */
    ...
    /**
     * Returns the public fields of the parameter <code>e</code>.  If <code>e</code> and parameter
     * <code>oe</code> have the same class, then returns parameter <code>ofields</code>, otherwise
     * ensures that <code>e</code> has at least as many fields as does parameter
     * <code>ofields</code>.
     */
    ...
    /**
     * Returns <code>true</code> if the <code>Entry</code> parameter <code>e</code> is an element of
     * the <code>Entry[]</code> array parameter <code>eSet</code>; returns <code>false</code>
     * otherwise.
     */
    ...
    /**
     * Returns <code>true</code> if the <code>Entry[]</code> array parameter <code>eSet1</code>
     * contains the <code>Entry[]</code> array parameter <code>eSet2</code>; returns
     * <code>false</code> otherwise. That is, this method determines if <code>eSet2</code> is a
     * subset of <code>eSet1</code>.
     */
    private static boolean contains(Entry[] eSet1, Entry[] eSet2) {
        if (eSet2 == null || eSet2.length == 0)
            return true;
        if (eSet1 == null || eSet1.length < eSet2.length)
            return false;

        for (int i = 0; i < eSet2.length; i++)
