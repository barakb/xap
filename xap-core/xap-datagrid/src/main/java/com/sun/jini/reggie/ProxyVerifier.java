...
final class ProxyVerifier implements TrustVerifier, Serializable {
    ...
    /**
     * Canonical service reference, used for comparison with inner server references extracted from
     * smart proxies to verify.
     *
     * @serial
     */
    private final RemoteMethodControl server;
    /**
     * The registrar's service ID, used for comparison with registrar service IDs extracted from
     * smart proxies to verify.
     */
    private transient ServiceID registrarID;

    /**
     * Constructs proxy verifier which compares server references extracted from smart proxies with
     * the given canonical server reference, which must implement both RemoteMethodControl and
     * TrustEquivalence.  For proxies which contain a copy of the registrar's service ID, that copy
     * is compared against the given service ID to ensure consistency.
     */
    ...
    /**
     * Returns true if the given object is a trusted proxy, or false otherwise. The given object is
     * trusted if it is trust equivalent to the canonical server reference carried by this trust
     * verifier, or if it is an instance of one of Reggie's constrainable smart proxy classes, and
     * all component proxies it contains are trusted, and its inner server reference is trust
     * equivalent to the canonical server reference, and its inner copy of the registrar's service
     * ID (if it has one) is equal to the service ID carried by this verifier.
     */
     ...
     * deserialized registrar reference implements both RemoteMethodControl and TrustEquivalence.
     */
