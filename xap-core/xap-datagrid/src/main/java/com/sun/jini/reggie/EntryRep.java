 ...
 * communication between the proxies and the server. <p> This class only has a bare minimum of
 * methods, to minimize the amount of code downloaded into clients.
 *
...
public class EntryRep implements Serializable, Cloneable {
    ...
    /**
     * The public fields of the Entry, each converted as necessary to a MarshalledWrapper (or left
     * as is if of known java.lang immutable type).  The fields are in super- to subclass order.
     *
     * @serial
     */
    ...
    /**
     * Converts an Entry to an EntryRep.  Any exception that results is bundled up into a
     * MarshalException.
     */
    ...
    /**
     * Convert back to an Entry.  If the Entry cannot be constructed, null is returned.  If a field
     * cannot be unmarshalled, it is set to null.
     */
    public Entry get() {
        try {
    ...
    public int hashCode() {
        int result;
        result = (eclass != null ? eclass.hashCode() : 0);
        result = 31 * result + (fields != null ? arrayHash(fields) : 0);
        return result;
    }

    private static int arrayHash(Object a[]) {
        if (a == null)
            return 0;
    ...
    public String toString() {
        ...
        sb.append("EntryRep[").append(eclass.getName()).append(",");
        if (fields != null) {
            sb.append(" fields [");
    ...
    /**
     * EntryReps are equal if they have the same class and the fields are pairwise equal.  This is
     * really only needed in the server, but it's very convenient to have here.
     */
    ...
    /**
     * Deep clone (which just means cloning the fields array too). This is really only needed in the
     * server, but it's very convenient to have here.
     */
    public Object clone() {
        try {
            EntryRep entry = (EntryRep) super.clone();
            entry.fields = entry.fields.clone();
            return entry;
    ...
    /**
     * Converts an array of Entry to an array of EntryRep.  If needCodebase is false, then the
     * codebase of every EntryRep will be null.
     */
