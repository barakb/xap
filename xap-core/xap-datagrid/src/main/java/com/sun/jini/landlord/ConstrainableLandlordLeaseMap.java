...
final public class ConstrainableLandlordLeaseMap extends LandlordLeaseMap {
     ...
     *
     * @param landlord     Reference to the entity that created <code>lease</code>.  Assumes that
     *                     any necessary method constraints have been attached.
     * @param landlordUuid a universally unique id that has been assigned to the server granting of
     *                     the lease. Ideally the <code>Uuid</code> {@link ReferentUuid.getUuid
     *                     landlord.getUuid} would return if <code>landlord</code> implemented
     *                     {@link ReferentUuid}. Used to determine when leases can be added to this
     *                     map.
     * @param lease        First lease to be placed in the map.  It is assumed that
     *                     <code>canContainKey(lease)</code> would be <code>true</code>.  Must work
     *                     with the landlord protocol.
     * @param duration     The duration the lease should be renewed for if <code>renewAll</code> is
     *                     called.
