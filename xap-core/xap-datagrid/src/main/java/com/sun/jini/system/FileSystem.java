...
public class FileSystem {
    /**
     * Remove this completely.  If the parameter is a directory, it is removed after recursively
     * destroying all its contents, including subdirectories.  If the named file does not exist,
     * <code>destroy</code> simply returns.
     *
     * @param proceed Proceed in the face of errors; otherwise the first error stops the execution
     *                of the method.
    ...
    /**
     * Handle an error, either by adding to the list, or if there is no list, throwing an
    ...
    /**
     * Ensure that the given path is a directory, creating it if necessary.  If the path exists it
     * must be a directory. It the path does not exist this method uses <code>File.mkdirs</code> to
     * create the directory along with any intermediate paths.
     *
     * @throws IllegalArgumentException if the path already exists but is not a directory, or it
