 ...
 * server, the references returned by that method should be the same.  This allows <code>==</code>
 * tests to work for proxies to remote servers the same as they would for direct references to
 * remote servers, which also maintain this property. <p> Here is an example that uses this class to
 * ensure that exactly one copy of a <code>java.io.Resolvable</code> object exists in each VM:
 * <pre>
...
public class WeakTable {
    ...
    /**
     * Create a new WeakTable object to maintain the maps that calls back the designated object when
     * keys are collected.
     */
    ...
    /**
     * Return the object that this key maps to.  If it currently maps to no object, set it to map to
     * the given object.  Return either the existing entry or the new one, whichever is used.
     */
    ...
    /**
     * Return the value associated with given key, or <code>null</code> if no value can be found.
     */
    ...
    /**
     * Remove the object that the given key maps to.  If found return the object, otherwise return
     * null.
     */
    ...
    /**
     * Remove any blank entries from the table.  This can be invoked by a reaping thread if you
     * like.
     */
    /*
