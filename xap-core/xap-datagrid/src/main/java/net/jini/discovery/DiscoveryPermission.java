 ...
 * in this case, group names are assumed to be in dotted domain name style (e.g., "*.sun.com"). <p>
 * Note that, due to limitations of the Java(TM) platform security model, permission must be granted
 * not only to the code that calls LookupDiscovery, but also to LookupDiscovery itself.
 *
...
public final class DiscoveryPermission extends Permission
    ...
    /**
     * Returns true if the name of this permission (the one on which the method is invoked) is the
     * same as the name of the permission parameter, or if the name of this permission starts with
     * '*' and the remainder (after the '*') is a suffix of the name of the permission parameter.
     */
