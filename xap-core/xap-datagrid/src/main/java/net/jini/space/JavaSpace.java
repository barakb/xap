...
public interface JavaSpace {
     ...
     *
     * Equivalent to calling {@link com.j_spaces.core.IJSpace#write(Object, Transaction, long, long,
     * int)} with {@link com.j_spaces.core.client.UpdateModifiers#WRITE_ONLY}.
     *
     * @param entry the entry to write
     * @param txn   the transaction object, if any, under which to perform the write
    ...
    /**
     * Read any matching entry from the space, blocking until one exists. Return <code>null</code>
     * if the timeout expires.
     *
     * @param tmpl    The template used for matching.  Matching is done against <code>tmpl</code>
     *                with <code>null</code> fields being wildcards ("match anything") other fields
     ...
     *                entry.  A timeout of <code>NO_WAIT</code> means to wait no time at all; this
     *                is equivalent to a wait of zero.
     ...
     * @throws TransactionException     if a transaction error occurs
     * @throws InterruptedException     if the thread in which the read occurs is interrupted
     * @throws RemoteException          if a communication error occurs
    ...
    /**
     * Read any matching entry from the space, returning <code>null</code> if there is currently is
    ...
    /**
     * Take a matching entry from the space, waiting until one exists. Matching is and timeout done
     ...
     *                                  deserialized for any reason
     * @throws TransactionException     if a transaction error occurs
     * @throws InterruptedException     if the thread in which the take occurs is interrupted
     * @throws RemoteException          if a communication error occurs
     * @throws IllegalArgumentException if a negative timeout value is used
     * @see #read
     */
    ...
    /**
     * Take a matching entry from the space, returning <code>null</code> if there is currently is
     * none.  Matching is and timeout done as for <code>read</code>, except that blocking in this
     * call is done only if necessary to wait for transactional state to settle.
     *
     * @param tmpl    The template used for matching.  Matching is done against <code>tmpl</code>
     *                with <code>null</code> fields being wildcards ("match anything") other fields
    ...
    /**
     * When entries are written that match this template notify the given <code>listener</code> with
     * a <code>RemoteEvent</code> that includes the <code>handback</code> object.  Matching is done
     * as for <code>read</code>.
     *
     * @param tmpl     The template used for matching.  Matching is done against <code>tmpl</code>
     *                 with <code>null</code> fields being wildcards ("match anything") other fields
     ...
     * @throws TransactionException     if a transaction error occurs
     * @throws RemoteException          if a communication error occurs
     * @throws IllegalArgumentException if the lease time requested is not Lease.ANY and is
     *                                  negative
     * @see #read
     ...
     * that are used repeatedly to search for entries with <code>read</code> or <code>take</code>.
     * The client-side implementations of <code>read</code> and <code>take</code> cannot reasonably
     ...
     *
     * The <code>snapshot</code> method gives the JavaSpaces service implementor a way to reduce the
     * impact of repeated use of the same entry. Invoking <code>snapshot</code> with an
     * <code>Entry</code> will return another <code>Entry</code> object that contains a
     * <i>snapshot</i> of the original entry. Using the returned snapshot entry is equivalent to
     ...
     * <code>snapshot</code> a <code>null</code> template; <code>snapshot</code> may or may not
     * return null given a <code>null</code> template.
     *
     * The entry returned from <code>snapshot</code> will be guaranteed equivalent to the original
     * unmodified object only when used with the space. Using the snapshot with any other JavaSpaces
     * service will generate an <code>IllegalArgumentException</code> unless the other space can use
     ...
     * code, and <code>equals</code> may or may not return <code>true</code> when invoked with the
