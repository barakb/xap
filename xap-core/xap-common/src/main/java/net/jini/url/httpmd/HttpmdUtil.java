...
public class HttpmdUtil {
    ...
    /**
     * A URL handler to use for HTTPMD URLs so we don't require the protocol handler to be
     * installed.
     */
     ...
     * @return the message digest, as a <code>String</code> in hexadecimal format
     * @throws IOException              if an I/O exception occurs while reading data from the URL
     * @throws NoSuchAlgorithmException if no provider is found for the message digest algorithm
    ...
    /**
     * Computes the message digests for a codebase with HTTPMD URLs. <p> Do not use a directory on a
     * remote filesystem, or a directory URL, if the underlying network access protocol does not
     * provide adequate data integrity or authentication of the remote host.
     *
     * @param sourceDirectory the filename or URL of the directory containing the source files
     *                        corresponding to the URLs in <code>codebase</code>
     * @param codebase        a space-separated list of HTTPMD URLs. The digest values specified in
     ...
     *                        relative to <code>sourceDirectory</code>, to use for computing message
     *                        digests.
     * @return the codebase updated to include computed digests
     * @throws IllegalArgumentException if any of the URLs in <code>codebase</code> fail to specify
     *                                  the HTTPMD protocol
     * @throws IOException              if an I/O exception occurs while reading data from the
     *                                  source files
     * @throws MalformedURLException    if any of the URLs in <code>codebase</code> have incorrect
     *                                  syntax
     * @throws NullPointerException     if either argument is <code>null</code>
     */
    ...
    /**
     * Returns true if the character is permitted in an HTTPMD comment.  Legal comment characters
     * are ASCII letters and numbers, plus '-', '_', '.', '~', '*', ''', '(', ')', ':', '@', '&',
     * '=', '+', '$', and ','.
     */
