...
public final class BasicUntrustedObjectSecurityContext
    ...
    /**
     * Constructs an instance with the specified permissions. The security context returned by
     * {@link #getContext getContext} will be restricted to the permissions granted by default to
     * all code plus the additional specified permissions. The argument passed to the constructor is
     * neither modified nor retained; subsequent changes to that argument have no effect on the
     * instance created.
     *
     * @param permissions additional permissions to allow, or <code>null</code> if no additional
     *                    permissions should be allowed
     * @throws NullPointerException if any element of the argument is <code>null</code>
     */
    ...
    /**
     * Returns a security context to use to restrict privileges when invoking methods on untrusted
     * objects. The returned context is equivalent to the current security context (as returned by
     * {@link Security#getContext Security.getContext}) with an additional protection domain
     * combined into the access control context that contains an empty {@link CodeSource}
     * (<code>null</code> location and certificates), a permission collection containing the
     * permissions used to construct this instance, <code>null</code> class loader, and
     * <code>null</code> principals.
     *
     * @return a security context to use to restrict privileges when invoking methods on untrusted
     * objects
     */
     ...
     * class that were constructed with equivalent permissions. The order of permissions in the
     * array that was passed to the constructor is not significant.
     */
