...
/**
 * Permission that is specially intepreted by {@link PolicyFileProvider} as shorthand for a {@link
 * net.jini.security.GrantPermission} covering all permissions authorized to a given protection
 * domain.  For each protection domain authorized with a set of permissions <code>ps</code> that
 * includes an <code>UmbrellaGrantPermission</code>, <code>PolicyFileProvider</code> adds to
 * <code>ps</code> a <code>GrantPermission</code> for all permissions in <code>ps</code>.  All
 * <code>UmbrellaGrantPermission</code> instances are equivalent, and have target names and action
 * strings that are the empty string.
 *
