 ...
 * {@link ProxyTrustVerifier}. The remote object to be exported (called the main remote object) must
 * be an instance of {@link ServerProxyTrust}. In addition to exporting the main remote object, this
 * exporter also exports a second remote object (called the bootstrap remote object, which is
 * created internally by this exporter) that implements the {@link ProxyTrust} interface by
 * delegating the {@link ProxyTrust#getProxyVerifier getProxyVerifier} method to the corresponding
 * {@link ServerProxyTrust#getProxyVerifier getProxyVerifier} method of the main remote object.
 *
...
public class ProxyTrustExporter implements Exporter {
     ...
     * bootstrap remote object). The main exporter must produce a proxy (called the main proxy) that
     * is an instance of both {@link RemoteMethodControl} and {@link TrustEquivalence}, and if the
     * main proxy's class is not <code>public</code>, the direct superinterfaces of that class and
     * all of its superclasses must be <code>public</code>. The bootstrap exporter, when used to
     * export a remote object that is an instance of {@link ProxyTrust}, must produce a proxy
     * (called the bootstrap proxy) that is an instance of <code>ProxyTrust</code>,
     * <code>RemoteMethodControl</code>, and <code>TrustEquivalence</code>, and should satisfy the
     * bootstrap proxy requirements of <code>ProxyTrustVerifier</code>. The dynamic proxy class
     * generated at export will be defined by the same class loader as the main proxy's class.
     *
     * @param mainExporter the main exporter, for the main remote object
     * @param bootExporter the bootstrap exporter, for the bootstrap remote object
     ...
     * will be defined). The main exporter must produce a proxy (called the main proxy) that is an
     * instance of both {@link RemoteMethodControl} and {@link TrustEquivalence}, and if the main
     * proxy's class is not <code>public</code>, the direct superinterfaces of that class and all of
     * its superclasses must be <code>public</code>. The bootstrap exporter, when used to export a
     * remote object that is an instance of {@link ProxyTrust}, must produce a proxy (called the
     * bootstrap proxy) that is an instance of <code>ProxyTrust</code>,
     * <code>RemoteMethodControl</code>, and <code>TrustEquivalence</code>, and should satisfy the
     * bootstrap proxy requirements of <code>ProxyTrustVerifier</code>. If the specified class
     * loader is <code>null</code>, the dynamic proxy class generated at export will be defined by
     * the same class loader as the main proxy's class.
     *
     * @param mainExporter the main exporter, for the main remote object
     * @param bootExporter the bootstrap exporter, for the bootstrap remote object
     * @param loader       the class loader to define the proxy class in, or <code>null</code>
     * @throws NullPointerException if either exporter argument is <code>null</code>
     */
    ...
    /**
     * Exports the specified main remote object and returns a dynamic proxy for the object. The main
     * remote object must be an instance of {@link ServerProxyTrust}. The main remote object is
     * exported using the main exporter (specified at the construction of this exporter), returning
     * a proxy called the main proxy. The main proxy must be an instance of both {@link
     * RemoteMethodControl} and {@link TrustEquivalence}, and if the main proxy's class is not
     * <code>public</code>, the direct superinterfaces of that class and all of its superclasses
     * must be <code>public</code>. A bootstrap remote object that is an instance of {@link
     * ProxyTrust} is created and exported using the bootstrap exporter (also specified at the
     * construction of this exporter), returning a proxy called the bootstrap proxy. The bootstrap
     * proxy must be an instance of <code>ProxyTrust</code>, <code>RemoteMethodControl</code>, and
     * <code>TrustEquivalence</code>. The bootstrap remote object will remain reachable as long as
     * the main remote object is reachable and the {@link #unexport unexport} method of this
     * exporter has not returned <code>true</code>. A {@link Proxy} class is generated that
     ...
     * <code>implements</code> clause); and if an interface appears more than once, only the first
     * instance is retained. If a non-<code>null</code> class loader was specified at the
     ...
     * this method is an instance of that generated class, containing a {@link
     * ProxyTrustInvocationHandler} instance created with the main proxy and the bootstrap proxy.
     *
     * @throws ExportException          if the export of either remote object throws
     *                                  <code>ExportException</code>, or if the export the bootstrap
     *                                  remote object throws <code>IllegalArgumentException</code>,
     *                                  or if the main proxy is not an instance of both
     *                                  <code>RemoteMethodControl</code> and <code>TrustEquivalence</code>,
     *                                  or if the main proxy's class is not <code>public</code> and
     *                                  it or a superclass has a non-<code>public</code> direct
     *                                  superinterface, or if the bootstrap proxy is not an instance
     *                                  of <code>ProxyTrust</code>, <code>RemoteMethodControl</code>,
     *                                  and <code>TrustEquivalence</code>, or if any of the
     *                                  superinterfaces of the main proxy's class are not visible
     *                                  through the class loader specified at the construction of
     *                                  this exporter
     * @throws IllegalArgumentException if the specified remote object is not an instance of
     *                                  <code>ServerProxyTrust</code>, or if the export of the main
     *                                  remote object throws <code>IllegalArgumentException</code>
     * @throws IllegalStateException    if the export of either remote object throws
     *                                  <code>IllegalStateException</code>
     * @throws SecurityException        if a non-<code>null</code> class loader was not specified at
     *                                  the construction of this exporter and the calling context
     *                                  does not have {@link RuntimePermission}<code>("getClassLoader")</code>
     *                                  permission
     */
    ...
    /**
     * Unexports the remote objects that were previously exported via this exporter. The
     * <code>unexport</code> method of the main exporter is called with the specified argument and
     * if that returns <code>true</code>, the <code>unexport</code> method of the bootstrap exporter
     * is called with <code>true</code>. The result of the main <code>unexport</code> call is
     * returned by this method. Any exception thrown by either <code>unexport</code> call is
     * rethrown by this method.
     *
     * @throws IllegalStateException if the unexport of either remote object throws
     *                               <code>IllegalStateException</code>
     */
    ...
    /**
     * Weak reference to the main remote object with strong reference to the bootstrap remote
     * object.
     */
    ...
    private static class Reaper implements Runnable {
        ...
        /**
         * Keep removing refs from the queue, clearing them, and removing them from the refs list,
