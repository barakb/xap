 ...
 * equivalent in trust, content, and function to a known trusted object. This interface can be
 * useful in implementing a {@link net.jini.security.TrustVerifier} returned by {@link
 * ProxyTrust#getProxyVerifier ProxyTrust.getProxyVerifier}. <p> An implementation of this interface
 * must always compare components of the two objects by invoking comparison methods (such as
 * <code>equals</code> or {@link #checkTrustEquivalence checkTrustEquivalence}) on components of the
 * known trusted object, and those comparison methods must in turn compare subcomponents in the same
 * fashion, always invoking comparison methods on components of <code>this</code>. <p> For example,
 ...
 * and use its <code>checkTrustEquivalence</code> method to check that a candidate smart proxy
...
public interface TrustEquivalence {
    /**
     * Returns <code>true</code> if the specified object (that is not yet known to be trusted) is
     * equivalent in trust, content, and function to this known trusted object, and returns
     * <code>false</code> otherwise. This method is permitted to delete content of the specified
     ...
     * content that has been checked) during subsequent operations on the object.
     *
