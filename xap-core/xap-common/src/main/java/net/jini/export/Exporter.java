 ...
 *
 * <p>Details of the export and unexport behavior, including the communication protocols used for
 * remote invocation and additional remote invocation semantics, are defined by the particular
...
public interface Exporter {
    ...
    /**
     * Unexports the remote object that was exported by this <code>Exporter</code> such that it will
     * no longer receive remote method invocations that were made possible as a result of exporting
     * it with this <code>Exporter</code>.  The unexport operation may not occur if the
     * <code>force</code> argument is <code>false</code>. This method must only be invoked after the
     * <code>export</code> method has been invoked on this <code>Exporter</code> instance to export
     * a remote object successfully.
     *
     * <p>This method returns <code>true</code> if upon return, the remote object is no longer
     * exported with this <code>Exporter</code>, and <code>false</code> if the remote object remains
     * exported with this <code>Exporter</code>.  This method will always return <code>true</code>
     * if it has returned <code>true</code> previously.
     *
     * <p>The <code>force</code> parameter serves to indicate whether or not the caller desires the
     * unexport to occur even if there are known remote calls pending or in progress to the remote
     * object that were made possible by this <code>Exporter</code>:
     *
     * <ul> <li>If <code>force</code> is <code>true</code>, then the remote object will be forcibly
     * unexported even if there are remote calls pending or in progress, and this method will return
     * <code>true</code>.
     *
     * <li>If <code>force</code> is <code>false</code>, then this acts as a hint to the
     ...
     * return <code>true</code> or not unexport the remote object and return <code>false</code>.  If
     * the implementation detects that there are indeed remote calls pending or in progress, then it
     * should return <code>false</code>; otherwise, it must return <code>true</code>. If the
     ...
     * <code>force</code> were always <code>true</code>. </ul>
     *
     * <p>If the remote object is unexported as a result of this method, then the implementation may
     * (and should, if possible) prevent remote calls in progress from being able to communicate
     * their results successfully.
     *
     * @param force if <code>true</code>, the remote object will be unexported even if there are
     *              remote calls pending or in progress; if <code>false</code>, the remote object
     *              may only be unexported if there are no known remote calls pending or in
     *              progress
     * @return <code>true</code> if the remote object is unexported when this method returns and
     * <code>false</code> otherwise
     * @throws IllegalStateException if an object has not been exported with this
     *                               <code>Exporter</code> instance
     **/
