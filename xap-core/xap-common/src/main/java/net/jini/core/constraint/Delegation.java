 ...
 * delegation.) <p> The use of an instance of this constraint does not directly imply a {@link
 * ClientAuthentication#YES} constraint; that must be specified separately to ensure that the client
 * actually authenticates itself. <p> Serialization for this class is guaranteed to produce
 * instances that are comparable with <code>==</code>.
 *
...
public final class Delegation implements InvocationConstraint, Serializable {
     ...
     * identity in subsequent remote calls that the server makes or receives. The mechanisms and
     * credentials used to support delegation are not specified by this constraint. Note that,
     * because this constraint is conditional on client authentication, it does not conflict with
     * {@link ClientAuthentication#NO}.
     */
