...
public interface NestableTransactionManager extends TransactionManager {
     ...
     *                                     longer active.
     * @throws LeaseDeniedException        if this manager is unwilling to grant the requested lease
     *                                     time
    ...
    /**
     * Promote the listed participants into the specified transaction. This method is for use by the
     * manager of a subtransaction when the subtransaction commits.  At this point, all participants
     * of the subtransaction must become participants in the parent transaction. Prior to this
     * point, the subtransaction's manager was a participant of the parent transaction, but after a
     * successful promotion it need no longer be one (if it was not itself a participant of the
     * subtransaction), and so it may specify itself as a participant to drop from the transaction.
     * Otherwise, participants should not be dropped out of transactions.  For each promoted
     * participant, the participant's crash count is stored in the corresponding element of the
     * <code>crashCounts</code> array.
     *
