 ...
 * <code>TransactionManager</code> interface, this interface allows a two-phase commit protocol to
 * be used between objects in a distributed system.
 *
...
public interface TransactionParticipant extends Remote, TransactionConstants {
     ...
     * outcome of the transaction. The participant responds with either <code>PREPARED</code>,
     * indicating that it is prepared; <code>ABORT</code>, indicating that it will abort, or
     * <code>NOTCHANGED</code>, indicating that it did not have any state changed by the transaction
     * (i.e., it was read-only). If the response is <code>PREPARED</code>, the participant must wait
     ...
     * <code>ABORT</code>, the participant should roll its state back to undo any changes that
     * occurred due to operations performed under the transaction; it can then discard any
     * information about the transaction. If the response is <code>NOTCHANGED</code>, the
    ...
    /**
     * Requests that the participant make all of its <code>PREPARED</code> changes for the specified
     ...
     * @param mgr               the manager of the transaction
     * @param id                the transaction ID
     * @param numOfParticipants the total number of transaction participants this is needed only in
     *                          commit , since prepareAndCommit is called for single participant
     * @throws UnknownTransactionException if the transaction is unknown to the transaction manager,
     *                                     either because the transaction ID is incorrect or because
     *                                     the transaction is complete and its state has been
     *                                     discarded by the manager.
     ...
     * any resources locked by the transaction. All state associated with the transaction can then
     * be discarded by the participant.
     *
     * @param mgr the manager of the transaction
     * @param id  the transaction ID
     * @throws UnknownTransactionException if the transaction is unknown to the transaction manager,
    ...
    /**
     * A combination of <code>prepare</code> and <code>commit</code>, which can be used by the
     * manager when there is just one participant left to prepare and all other participants (if
     * any) have responded with <code>NOTCHANGED</code>.  The participant's implementation of this
     ...
     */
    @UseStubCache
    void renewLease(TransactionManager mgr, long id, long time)
