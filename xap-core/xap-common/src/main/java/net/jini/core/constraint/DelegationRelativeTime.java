 ...
 * for a range of time measured relative to the start of the remote call. The mechanisms and
 * credentials used to support this are not specified by this constraint. Each end of the range is
 * itself specified as a range, yielding four durations, all specified in milliseconds. If
 * <code>t</code> represents the current time at the start of a remote call, then the four durations
 * have the following semantics: <ul> <li><code>minStart</code> - delegation must not be permitted
 * any earlier than time <code>t + minStart</code> <li><code>maxStart</code> - delegation must be
 * permitted from time <code>t + maxStart</code> onwards <li><code>minStop</code> - delegation must
 * be permitted up until at least time <code>t + minStop</code> <li><code>maxStop</code> -
 * delegation must not be permitted after time <code>t + maxStop</code> </ul> The durations are
 * translated into absolute end times at the point of a remote call by adding the caller's current
 * time. To accommodate clock skew between systems, it is permitted (and may be desirable) to
 * specify negative values for the minimum and maximum start durations, to include start times that
 * are earlier than the current time. <p> The use of an instance of this class does not directly
 * imply a {@link ClientAuthentication#YES} constraint or a {@link Delegation#YES} constraint; those
 * must be specified separately to ensure that the client actually authenticates itself and that
 * delegation is actually used. Because this constraint is conditional on delegation, it does not
 * conflict with {@link ClientAuthentication#NO} or {@link Delegation#NO}.
 *
...
public final class DelegationRelativeTime
     ...
     * @throws IllegalArgumentException if <code>minStart</code> is greater than <code>maxStart</code>,
     *                                  or <code>maxStart</code> is greater than <code>minStop</code>,
     *                                  or <code>minStop</code> is greater than <code>maxStop</code>,
     *                                  or <code>minStop</code> is less than zero
     */
    ...
    /**
     * Returns a {@link DelegationAbsoluteTime} instance with times obtained by adding the specified
     * base time argument to the duration values from this instance. If an addition results in
     * underflow or overflow, a time value of <code>Long.MIN_VALUE</code> or
    ...
    /**
     * Verifies that <code>minStart</code> is less than or equal to <code>maxStart</code>,
     * <code>maxStart</code> is less than or equal to <code>minStop</code>, <code>minStop</code> is
     * less than or equal to <code>maxStop</code>, and <code>minStop</code> is greater than or equal
     * to zero.
     *
     * @throws InvalidObjectException if <code>minStart</code> is greater than <code>maxStart</code>,
     *                                or <code>maxStart</code> is greater than <code>minStop</code>,
     *                                or <code>minStop</code> is greater than <code>maxStop</code>,
     *                                or <code>minStop</code> is less than zero
     */
