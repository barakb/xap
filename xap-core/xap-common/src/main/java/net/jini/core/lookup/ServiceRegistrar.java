...
/**
 * Defines the interface to the lookup service.  The interface is not a remote interface; each
 * implementation of the lookup service exports proxy objects that implement the ServiceRegistrar
 * interface local to the client, using an implementation-specific protocol to communicate with the
 * actual remote server.  All of the proxy methods obey normal RMI remote interface semantics except
 * where explicitly noted.  Two proxy objects are equal if they are proxies for the same lookup
 * service. Every method invocation (on both ServiceRegistrar and ServiceRegistration) is atomic
...
public interface ServiceRegistrar {
     ...
     * can simply repeat the call to register with the same parameters, until it succeeds. <p> To
     ...
     * cancelled, but that item's service ID is reused for the newly registered item. <p> To
     ...
     * or the service implements a new interface). <p> Any duplicate attribute sets included in a
     ...
     * template, or null if there is no match.  If multiple items match the template, it is
     * arbitrary as to which service object is returned.  If the returned object cannot be
     * deserialized, an UnmarshalException is thrown with the standard RMI semantics.
     *
     ...
     * match the template.  The return value is never null, and the returned items array is only
     * null if maxMatches is zero. For each returned item, if the service object cannot be
     * deserialized, the service field of the item is set to null and no exception is thrown.
     * Similarly, if an attribute set cannot be deserialized, that element of the attributeSets
     * array is set to null and no exception is thrown.
     *
     * @param tmpl       template to match
     * @param maxMatches maximum number of matches to return
     * @return a ServiceMatches instance that contains at most maxMatches items matching the
     * template, plus the total number of items that match the template.  The return value is never
    ...
    /**
     * An event is sent when the changed item matches the template before the operation, but doesn't
     * match the template after the operation (this includes deletion of the item).
     */
    int TRANSITION_MATCH_NOMATCH = 1;

    /**
     * An event is sent when the changed item doesn't match the template before the operation (this
     * includes not existing), but does match the template after the operation.
     */
    int TRANSITION_NOMATCH_MATCH = 1 << 1;

    /**
     * An event is sent when the changed item matches the template both before and after the
     * operation.
     */
    int TRANSITION_MATCH_MATCH = 1 << 2;

    /**
     * Registers for event notification.  The registration is leased; the lease expiration request
     * is not exact.  The registration is persistent across restarts (crashes) of the lookup service
     * until the lease expires or is cancelled.  The event ID in the returned EventRegistration is
     * unique at least with respect to all other active event registrations in this lookup service
     * with different service templates or transitions. <p> While the event registration is in
     * effect, a ServiceEvent is sent to the specified listener whenever a register, lease
     * cancellation or expiration, or attribute change operation results in an item changing state
     * in a way that satisfies the template and transition combination.
     *
     * @param tmpl          template to match
     ...
     * @return an EventRegistration object to the entity that registered the specified remote
     * listener
     */
    ...
    int NOTIFY_WITH_MATCHES = 1;

    int NOTIFY_WITH_EVENTS = 2;
     ...
     *
     * @param tmpl template to match
     * @return an array of entry Classes (attribute sets) for every service that matches the
     * specified template
     */
     ...
     * values of the specified field of those entries.  Duplicate values are eliminated, and the
     * order of values within the returned array is arbitrary.  Null (not an empty array) is
     * returned if there are no matching items.  If a returned value cannot be deserialized, that
     * element of the returned array is set to null and no exception is thrown.
     *
     * @param tmpl     template to match
     * @param setIndex index into tmpl.attributeSetTemplates
     * @param field    name of field of tmpl.attributeSetTemplates[setIndex]
     * @return an array of objects that represents field values of entries associated with services
     * that meet the specified matching criteria
     * @throws NoSuchFieldException field does not name a field of the entry template
     */
     ...
     * @return an array of Classes of all services that either match the specified template or match
    ...
    /**
     * Returns the service ID of the lookup service.  Note that this does not make a remote call.  A
     * lookup service is always registered with itself under this service ID, and if a lookup
     * service is configured to register itself with other lookup services, it will register with
     * all of them using this same service ID.
     *
     ...
     * service.
     *
     * @return a LookupLocator that can be used for unicast discovery of the lookup service, if
     * necessary.
     */
     ...
     *
     * @return a String array of groups that this lookup service is currently a member of.
     */
