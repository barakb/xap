...
/**
 * Class implementing the <code>NestableTransaction</code> interface, for use with transaction
...
public class NestableServerTransaction extends ServerTransaction
     ...
     * <code>TransactionFactory</code> and <code>NestableTransaction</code> methods.
     *
    ...
    public NestableServerTransaction(NestableTransactionManager mgr, long id,
                                     NestableServerTransaction parent, long lease) {
        super(mgr, id, lease);
        this.parent = parent;
    }

    // inherit javadoc
    public NestableTransaction.Created create(NestableTransactionManager mgr, long leaseTime)
            throws UnknownTransactionException, CannotJoinException,
            LeaseDeniedException, RemoteException {
        TransactionManager.Created rawTxn = mgr.create((NestableTransactionManager) this.mgr, id, leaseTime);
     ...
     * This method is for use by the manager of a subtransaction when the subtransaction commits. At
     * this point, all participants of the subtransaction must become participants in the parent
     * transaction.  Prior to this point, the subtransaction's manager was a participant of the
     * parent transaction, but after a successful promotion it need no longer be one (if it was not
     * itself a participant of the subtransaction), and so it may specify itself as a participant to
     * drop from the transaction.  Otherwise, participants should not be dropped out of
     * transactions.  For each promoted participant, the participant's crash count is stored in the
     ...
     * @throws CrashCountException         if the crash count provided for at least one participant
     *                                     differs from the crash count in a previous invocation of
     *                                     the same pairing of participant and transaction
     * @throws UnknownTransactionException if a transaction is unknown to the transaction manager,
     ...
     * @throws CannotJoinException         if a transaction is known to the manager but is no longer
     *                                     active.
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        if (in.readBoolean()) {
            parent = (NestableServerTransaction) in.readObject();
        }
    }
}
