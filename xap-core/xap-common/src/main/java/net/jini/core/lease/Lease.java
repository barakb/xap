...
public interface Lease {
    ...
    /**
     * Used by the requestor to indicate that there is no particular lease time desired, and that
     * the grantor of the lease should supply a time that is most convenient for the grantor.
     */
    long ANY = -1;
    ...
    /**
     * The serialized form of the lease will contain the time of expiration stored as an absolute
     * time, represented in terms of milliseconds since the beginning of the epoch.
     */
    int ABSOLUTE = 2;

    /**
     * Returns a <code>long</code> that indicates the time that the lease will expire. This time is
     * represented as milliseconds from the beginning of the epoch, relative to the local clock.
     *
     * @return a <code>long</code> that indicates the time that the lease will expire
     */
    long getExpiration();
     ...
     *
     * @throws UnknownLeaseException the lease being cancelled is unknown to the lease grantor
     */
     ...
     * @throws LeaseDeniedException  the lease grantor is unable or unwilling to renew the lease
     * @throws UnknownLeaseException the lease being renewed is unknown to the lease grantor
     */
     ...
     *
     * @param duration the duration to put into a Long and use as the value for the current lease in
     ...
     * @return a boolean indicating whether or not the lease given as a parameter can be batched
