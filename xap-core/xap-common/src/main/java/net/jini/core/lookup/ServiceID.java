...
/**
 * A universally unique identifier (UUID) for registered services. A service ID is a 128-bit value.
 * Service IDs are normally intended to either be built into services at manufacture or deployment
 * time, or generated dynamically by lookup services at registration time. <p>
 *
 ...
 * </pre>
 * The variant field must be 0x2. The version field must be either 0x1 or 0x4. If the version field
 * is 0x4, then the remaining fields are set to values produced by a cryptographically secure random
 * sequence.  If the version field is 0x1, then the node field is set to an IEEE 802 address, the
 * clock_seq field is set to a 14-bit random number, and the time_low, time_mid, and time_hi fields
 * are set to the least, middle and most significant bits (respectively) of a 60-bit timestamp
...
public final class ServiceID implements Serializable {
     ...
     * @throws IOException if there is a problem reading the most and least significant bits
     */
    ...
    /**
     * Returns a 36-character string of five fields separated by hyphens, with each field
     * represented in lowercase hexadecimal with the same number of digits as in the field. The
     * order of fields is: <code>time_low</code>, <code>time_mid</code>, <code>version</code> and
     * <code>time_hi</code> treated as a single field, <code>variant</code> and
