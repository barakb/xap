 ...
 * and the new state of the item. <p> Sequence numbers for a given event ID are strictly increasing.
 * If there is no gap between two sequence numbers, no events have been missed; if there is a gap,
 * events might (but might not) have been missed.  For example, a gap might occur if the lookup
 * service crashes, even if no events are lost due to the crash.
 *
...
public abstract class ServiceEvent extends net.jini.core.event.RemoteEvent {
    ...
    /**
     * Returns a <code>String</code> representation of this <code>ServiceEvent</code>.
     *
