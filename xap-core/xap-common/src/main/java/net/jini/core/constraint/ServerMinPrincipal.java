 ...
 * used to authenticate the server as those principals are not specified by this constraint. <p> The
 * use of an instance of this class does not imply directly a {@link ServerAuthentication#YES}
 ...
 * with {@link ServerAuthentication#NO}. <p> It is important to understand that specifying
 * <code>ServerAuthentication.YES</code> as a requirement <i>does not</i> ensure that a server is to
 * be trusted; it <i>does</i> ensure that the server authenticates itself as someone, but it does
 ...
 * generally needs to specify a <code>ServerMinPrincipal</code> requirement in addition, or else
 * verify that the server has specified a satisfactory <code>ServerMinPrincipal</code> requirement
