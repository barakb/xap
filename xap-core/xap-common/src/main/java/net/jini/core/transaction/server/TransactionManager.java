 ...
 * <code>ServerTransaction</code> objects are governed by a transaction manager that runs this
 * protocol.
 *
...
public interface TransactionManager extends Remote, TransactionConstants {
     ...
     *
     * @return true if its a  mgr  which requires the txn participants to join
     */
    boolean needParticipantsJoin() throws RemoteException;


    /**
     * Join a transaction that is managed by this transaction manager. The <code>crashCount</code>
     ...
     * storage must cause the crash count to increase by at least one.
     *
     * @param id         the transaction ID
    ...
    /**
     * Returns the current state of the given transaction.  The returned state can be any of the
     ...
     *                                     the transaction is complete and its state has been
     *                                     discarded by the manager.
     * @throws CannotCommitException       if the transaction is being or already has been aborted
     * @throws RemoteException             if there is a communication error
     */
    void commit(long id)
     ...
     *                                     discarded by the manager.
     * @throws CannotCommitException       if the transaction is being or already has been aborted
     * @throws TimeoutExpiredException     if the transaction manager reaches a decision, but is
    ...
    Uuid getTransactionManagerId() throws RemoteException;
}
