 ...
 * (using {@link RMIClassLoader}).  A <code>MarshalInputStream</code> is intended to read data
 * written by a corresponding {@link MarshalOutputStream}.
 *
 * <p><code>MarshalInputStream</code> implements the input side of the dynamic class loading
 ...
 * optionally with codebase annotation strings written by a <code>MarshalOutputStream</code>.
 *
 * <p>By default, a <code>MarshalInputStream</code> ignores all codebase annotation strings, instead
 * using a codebase value of <code>null</code> when loading classes.  Codebase annotation strings
 * will only be used by a <code>MarshalInputStream</code> after its {@link #useCodebaseAnnotations
 * useCodebaseAnnotations} method has been invoked.
 *
 * <p><code>MarshalInputStream</code> supports optional verification that all codebase annotation
 * URLs that are used to load classes resolved by the stream provide content integrity (see {@link
 * Security#verifyCodebaseIntegrity Security.verifyCodebaseIntegrity}).  Whether or not a particular
 * <code>MarshalInputStream</code> instance verifies that codebase annotation URLs provide content
 * integrity is determined by the <code>verifyCodebaseIntegrity</code> constructor argument.  See
 ...
 *
 * <p><code>MarshalInputStream</code> reads class annotations from its own stream; a subclass can
 * override the {@link #readAnnotation readAnnotation} method to read the class annotations from a
 * different location.
 *
 * <p>A <code>MarshalInputStream</code> is not guaranteed to be safe for concurrent use by multiple
 * threads.
 *
...
public class MarshalInputStream
    ...
    /**
     * if false, pass null codebase values to loadClass and loadProxyClass methods; if true, pass
     * codebase values from stream class annotations
     **/
    private boolean usingCodebaseAnnotations = false;

    /**
     * Creates a new <code>MarshalInputStream</code> that reads marshalled data from the specified
     ...
     *                                <code>ClassLoading</code> methods
     * @param verifyCodebaseIntegrity if <code>true</code>, this stream will verify that codebase
     *                                annotation URLs used to load classes resolved by this stream
     ...
     * @param context                 the collection of context information objects to be returned
     *                                by this stream's {@link #getObjectStreamContext
    ...
    /**
     * Enables the use of codebase annotation strings written by the corresponding
     * <code>MarshalOutputStream</code>.
     *
     * <p>If this method has not been invoked on this stream, then the <code>resolveClass</code> and
     * <code>resolveProxyClass</code> methods ignore the class annotation strings and just use a
     * <code>null</code> codebase value when loading classes.  After this method has been invoked,
     * then the <code>resolveClass</code> and <code>resolveProxyClass</code> methods use the class
     * annotation strings as codebase values.
     **/
     ...
     *
     * <p>This method first invokes this stream's {@link #readAnnotation readAnnotation} method to
     * read the annotation string value (possibly <code>null</code>) for the class descriptor.  If
     ...
     * of the class described by <code>classDesc</code> equals the Java programming language keyword
     * for a primitive type or <code>void</code>, then this method returns the <code>Class</code>
     * corresponding to that primitive type or <code>void</code> ({@link Integer#TYPE} for
     * <code>int</code>, {@link Void#TYPE} for <code>void</code>, and so forth). Otherwise, if
     * <code>ClassLoading.loadClass</code> throws an exception, this method throws that exception,
     * and if it returns normally, this method returns the <code>Class</code> returned by
     * <code>ClassLoading.loadClass</code>.
     *
     ...
     * value chosen is <code>null</code>.
     *
     * <p>This method then invokes {@link ClassLoading#loadProxyClass ClassLoading.loadProxyClass}
     * with the chosen codebase value as the first argument, <code>interfaceNames</code> as the
     * second argument, and the <code>defaultLoader</code>, <code>verifyCodebaseIntegrity</code>,
     * and <code>verifierLoader</code> values that were passed to this stream's constructor as the
     * third, fourth, and fifth arguments. If <code>ClassLoading.loadProxyClass</code> throws an
     * exception, then this method throws that exception.  Otherwise, this method returns the
     * <code>Class</code> returned by <code>ClassLoading.loadProxyClass</code>.
     *
