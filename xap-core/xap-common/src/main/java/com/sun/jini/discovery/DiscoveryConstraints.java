...
/**
 * Class for processing constraints which apply to the discovery protocol: {@link
 * DiscoveryProtocolVersion}, {@link MulticastMaxPacketSize}, {@link MulticastTimeToLive}, {@link
 * UnicastSocketTimeout}, {@link net.jini.core.constraint.ConnectionRelativeTime}, {@link
...
public class DiscoveryConstraints {
    ...
    /**
     * Empty method which serves as a {@link MethodConstraints} key for looking up {@link
     * InvocationConstraints} that apply to multicast requests.
     */
    ...
    /**
     * Empty method which serves as a {@link MethodConstraints} key for looking up {@link
     * InvocationConstraints} that apply to multicast announcements.
     */
    ...
    /**
     * Empty method which serves as a {@link MethodConstraints} key for looking up {@link
     * InvocationConstraints} that apply to unicast discovery.
     */
    public static void unicastDiscovery() {
    }

    /**
     * Processes the discovery-related constraints in the given set of constraints, returning a
     * <code>DiscoveryConstraints</code> instance from which the constraint results can be queried.
     * Processing of timeout constraints is time dependent, and subsequent invocations of this
     * method with the same <code>InvocationConstraints</code> may result in
     * <code>DiscoveryConstraint</code> instances with different constraint results.
     *
     * @param constraints the constraints to process
     * @return an instance representing processed constraints
     * @throws UnsupportedConstraintException if the discovery-related constraints contain
     *                                        conflicts, or otherwise cannot be processed
    ...
    /**
     * Returns the protocol version to use for sending multicast requests or announcements, or
     * initiating unicast discovery.
     *
    ...
    /**
     * Checks the protocol version of an incoming multicast request, announcement, or unicast
     * discovery attempt, throwing an {@link UnsupportedConstraintException} if handling of the
     * given protocol does not satisfy the constraints of this instance.
     *
     * @param version protocol version to check
     * @throws UnsupportedConstraintException if handling of the given protocol does not satisfy the
    ...
    /**
     * Returns the deadline by which a network connection must be established during unicast
     * discovery, or <code>defaultValue</code> if not constrained.
     *
    ...
    /**
     * Returns the maximum multicast packet size to allow, or the specified default value if not
     * constrained.
     *
     * @param defaultValue the value to return if the multicast packet size is unconstrained
    ...
    /**
     * Returns the multicast time to live value to use, or the specified default value if not
     * constrained.
     *
     * @param defaultValue the value to return if the multicast time to live value is unconstrained
    ...
    /**
     * Returns socket read timeout to use for unicast discovery, or the specified default value if
     * not constrained.
     *
     * @param defaultValue the value to return if the socket timeout is unconstrained
    ...
    /**
     * Returns the constraints for this instance which are not, or do not contain as alternatives,
     * instances of the "fulfillable" (by this layer) constraint types
     * <code>DiscoveryProtocolVersion</code>, <code>ConnectionRelativeTime</code>,
     * <code>MulticastMaxPacketSize</code>, <code>MulticastTimeToLive</code>, and
     * <code>UnicastSocketTimeout</code>. Constraint alternatives containing both fulfillable and
     * unfulfillable constraints are treated optimistically--it is assumed that the one of the
     * fulfillable alternatives will be satisfied, so the unfulfillable alternatives are not
     * included in the returned constraints.
     *
    ...
    /**
     * Utility for reducing constraints of a given type into a base set of alternatives.  For each
     * type of constraints that it reduces, this class makes the simplifying assumption that failure
     * to reduce constraints of that type into a single (perhaps one element) set of alternatives
     * represents a constraint conflict--i.e., that two disjoint instances of the constraint type
     * cannot be applied to the same operation.  While this restriction does not hold across all
     * possible constraints, it is satisfied by the particular constraint types that
     * DiscoveryConstraints handles.
     *
     * If this utility encounters a set of alternatives containing instances of both the type to
     * reduce as well as other constraint types, then it ignores the instances of the other types,
     * treating the set as if it can only be satisfied by the alternatives of the targeted type.
     * This may in some cases cause false positives when detecting conflicts in constraints
     * containing alternatives of mixed type; however, it should never result in false negatives.
     */
    private static class ConstraintReducer {
        ...
        /**
         * Returns the reduction of the given constraints into a single set of alternatives for the
         * target class.  Returns an empty set if no constraints of the target class are specified.
         * Throws UnsupportedConstraintException if the constraints conflict.
         */
        ...
        /**
         * Returns the reduction (intersection and compaction) of a new set of alternative
         * constraints, all instances of the target class, with a previously reduced set (null if no
         * other constraints have been reduced yet).  Returns an empty set if elements in the sets
         * conflict.  This method can be overridden by subclasses for constraints with particular
         * reduction semantics; the default implementation of this method returns the intersection
         * of the two sets.
         */
