...
public abstract class AbstractLeaseMap implements LeaseMap {
    ...
    /**
     * We use an AbstractSet to minimize the number of places where we have to wrap objects inside
     * new classes.  This could be expensive, but the standard underlying maps (HashMap and TreeMap)
     * also use an AbstractSet for this set, so we're really not making things that much worse.
     */
