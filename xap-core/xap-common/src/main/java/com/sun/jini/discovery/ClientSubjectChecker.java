 ...
 * multicast requests.  Instances of this interface are passed to the {@link
 * Discovery#decodeMulticastRequest decodeMulticastRequest} and {@link
 * Discovery#handleUnicastDiscovery handleUnicastDiscovery} methods of the {@link Discovery} class
 * as a means of controlling whether or not data can be accepted from or exchanged with a client
 * authenticated as the given subject.
 *
...
public interface ClientSubjectChecker {
     ...
     * as the given subject.  The passed subject is <code>null</code> if the client is
     * unauthenticated; if non-<code>null</code>, it must be read-only.  Returns normally if the
