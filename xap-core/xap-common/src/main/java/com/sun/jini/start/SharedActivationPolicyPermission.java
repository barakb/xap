...
/**
 * {@link Permission} class used by the {@linkplain com.sun.jini.start service starter} package.
 * This class takes a policy string argument that follows the matching semantics defined by {@link
 * FilePermission}. The {@link ActivateWrapper} class explicitly checks to see if the service's
 * import codebase has been granted access to service's associated policy file in the shared VM's
 * policy. <P> An example grant is:
 * <blockquote><pre>
 ...
 * };
 * </pre></blockquote>
 * This grant allows services using <code><VAR><B>install_dir</B></VAR>/lib/fiddler.jar</code> for
 * their import codebase to use <code><VAR><B>policy_dir</B></VAR>${/}policy.fiddler</code> for
 * their policy file, where <VAR><B>install_dir</B></VAR> is the installation directory of the
 * starter kit and <VAR><B>policy_dir</B></VAR> is the pathname to the directory containing the
 * policy file.
 *
...
public final class SharedActivationPolicyPermission extends Permission
    ...
    /**
     * <code>FilePermission</code> object that is the delegation target of the
     * <code>implies()</code> checks.
     *
     * @serial
     */
    ...
    /**
     * Constructor that creates a <code>SharedActivationPolicyPermission</code> with the specified
     * name. Delegates <code>policy</code> to supertype.
     */
    ...
    /**
     * Constructor that creates a <code>SharedActivationPolicyPermission</code> with the specified
     * name. This constructor exists for use by the <code>Policy</code> object to instantiate new
     * Permission objects. The <code>action</code> argument is currently ignored.
     */
