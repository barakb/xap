...
public class TObjectIntHashMap<K> extends TObjectHash<K> implements Externalizable {
    ...
    /**
     * Creates a new <code>TObjectIntHashMap</code> instance with a prime capacity equal to or
    ...
    /**
     * Creates a new <code>TObjectIntHashMap</code> instance whose capacity is the next highest
    ...
    public void putAll(TObjectIntHashMap<K> map) {
        for (TObjectIntIterator<K> it = map.iterator(); it.hasNext(); ) {
            put(it.key(), it.value());
        }
    }
    ...
    public TObjectIntHashMap<K> clone() {
        TObjectIntHashMap<K> clone = (com.gigaspaces.internal.gnu.trove.TObjectIntHashMap<K>) super.clone();
    ...
    /**
     * Adjusts the primitive value mapped to the key if the key is present in the map. Otherwise,
     * the <tt>initial_value</tt> is put in the map.
     *
     * @param key           the key of the value to increment
