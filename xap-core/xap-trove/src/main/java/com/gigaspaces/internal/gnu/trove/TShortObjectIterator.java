...
/**
 * Iterator for maps of type short and Object. <p/> <p>The iterator semantics for Trove's primitive
 * maps is slightly different from those defined in <tt>java.util.Iterator</tt>, but still well
 * within the scope of the pattern, as defined by Gamma, et al.</p> <p/> <p>This iterator does
 * <b>not</b> implicitly advance to the next entry when the value at the current position is
 * retrieved.  Rather, you must explicitly ask the iterator to <tt>advance()</tt> and then retrieve
 * either the <tt>key()</tt>, the <tt>value()</tt> or both.  This is done so that you have the
 ...
 * to the key/value parts of the current map entry happens in constant time.</p> <p/> <p>In
 ...
 * finger.</p> <p/> <p>Here are some sample scenarios for this class of iterator:</p> <p/>
 * <pre>
...
public class TShortObjectIterator<V> extends TPrimitiveIterator {
    ...
    /**
     * Replace the value of the mapping at the iterator's position with the specified value. Note
     * that you must <tt>advance()</tt> the iterator at least once before invoking this method.
     *
