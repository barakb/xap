 ...
 * objects).
 *
 * <p> Note that iteration is fastest if you forego the calls to <tt>hasNext</tt> in favor of
 * checking the size of the structure yourself and then call next() that many times:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>You may, of course, use the hasNext(), next() idiom too if you aren't in a performance
 * critical spot.</p>
 */
abstract class THashIterator<V> extends TIterator implements Iterator<V> {
     ...
     * @throws ConcurrentModificationException if the structure was changed using a method that
     *                                         isn't on this iterator.
